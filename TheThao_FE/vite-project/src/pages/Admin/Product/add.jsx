...
//         (async () => {
//             try {
//                 const r = await fetch(`${API_BASE}/categories`, { headers: { Accept: "application/json" } });
...
//         (async () => {
//             try {
//                 const r = await fetch(`${API_BASE}/brands?status=active`, { headers: { Accept: "application/json" } });
//                 const ct = r.headers.get("content-type") || "";
//                 const raw = await r.text();
//                 if (!r.ok) throw new Error(`HTTP ${r.status}: ${raw.slice(0, 200)}`);
//                 if (!ct.includes("application/json")) throw new Error(`Server không trả JSON: ${raw.slice(0, 200)}`);
